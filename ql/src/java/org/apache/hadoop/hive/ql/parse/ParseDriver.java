...
public class ParseDriver {
  ...
  /**
   * 该类为antlr的词法分析部分提供和实现不区分大小写的令牌检查。通过在检查词法规则时将令牌流转换为大写字母，这个类确保了词法规则只需要
   * 将令牌与大写字母匹配，而不是将大写字母和小写字母组合在一起。这纯粹用于匹配词法规则。实际的令牌文本以与用户输入相同的方式存储，而不
   * 实际地将其转换为大写字母。令牌值是由超级类ANTLRStringStream的consumption()函数生成的。LA()函数是前瞻函数，纯粹用于匹配词法规则。
   * 这也意味着语法只接受大写的令牌，以防它从其他工具中运行，如antlrworks没有antlrnocase stringstream实现。
   */
